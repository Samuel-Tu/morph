...
describe("Collapse.vue", () => {
  beforeAll(() => {
    ...
    contents = wrapper.findAll(".m-collapse-item__wrapper");
  ...
  test("test the infrastructure and corresponding text", () => {
    // lenght
  ...
  test("send the correct event", () => {
    expect(onChange).toHaveBeenCalledTimes(2);
    expect(onChange).toHaveBeenCalledWith([]);
    expect(onChange).toHaveBeenLastCalledWith(["b"]);
  });

  test("test disabled content", async () => {
    // disabled
  ...
  test("handle accordion mode error", () => {
    wrapper = mount(
      () => (
        <Collapse
          accordion
